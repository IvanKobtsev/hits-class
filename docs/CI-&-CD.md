...
1. Builds and tags the Docker image. You could do this on your PC by running `yarn docker-build`, or `build-with-docker` (if you want to build backend, frontend and image at once). [Dockerfile](https://github.com/mccsoft/backend-frontend-template/blob/master/webapi/src/Team13.HitsClass.App/Dockerfile) is taken from the webapi/src/App project.
