...
The easiest thing is to actually check the sources of [PatchProductDto](../webapi/src/Team13.HitsClass.App/Features/Products/Dto/PatchProductDto.cs) and [Patch method of ProductService](../webapi/src/Team13.HitsClass.App/Features/Products/Dto/PatchProductDto.cs).
...
For more details you could check an implementation of `Update` extension method in [PartialUpdateHelper](../webapi/Lib/WebApi/Team13.WebApi/Patching/PartialUpdateHelper.cs).
...
2. There's a test that verifies, that PatchDtos only contain the properties that exist in the Entity. That's helpful to avoid issues when you rename entity properties.
   1. To add an exception for certain Dtos/fields just modify the test `PatchRequest_AllFieldsMatch` in [BasicApiTests.cs](../webapi/tests/Team13.HitsClass.ComponentTests/BasicApiTests.cs).
