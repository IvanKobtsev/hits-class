...
const replacements = [
  ...
  { find: /team13:/g, replace: `${companyName.toLowerCase()}:` },
  { find: `${companyName}.IntegreSql.EF`, replace: `MccSoft.IntegreSql.EF` },
  { find: 'HitsClass', replace: projectName },
  { find: 'hitsclass', replace: projectName.toLowerCase() },
  {
    find: 'hitsClass',
    replace:
...
await fs.rename(
  'webapi/.idea/.idea.Team13.HitsClass',
...
function changeFrontendPortNumber(port: string | number) {
  ...
  replace({
    ...
    paths: ['./webapi/src/Team13.HitsClass.App/Team13.HitsClass.App.csproj'],
    silent: true,
    recursive: true,
  });
  replace({
    ...
    paths: ['./webapi/src/Team13.HitsClass.App/appsettings.json'],
    silent: true,
    recursive: true,
  });
  replace({
    regex: /https:\/\/localhost:\d+/g,
    replacement: `https://localhost:${port}`,
    paths: ['./webapi/src/Team13.HitsClass.App/appsettings.Development.json'],
    silent: true,
    recursive: true,
  });
  replace({
...
function changeBackendPortNumber(httpsPort: number) {
  ...
  replace({
    regex: /"https:\/\/localhost:\d+"/g,
    replacement: `"https://localhost:${httpsPort}"`,
    paths: ['./webapi/src/Team13.HitsClass.App/Properties/launchSettings.json'],
    silent: true,
  });
  replace({
    regex: /"https:\/\/\*:\d+"/g,
