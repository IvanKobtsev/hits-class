...
namespace Team13.PersistenceHelpers.DomainEvents;
    ...
    )
    {
        foreach (var entity in domainEventEntities)
        {
            ...
            foreach (var domainEvent in events)
            {
                ...
                {
                    domainEventWithStringId.Id = GetPrimaryKey<object>(context, entity).ToString();
                }
                ...
                {
                    logger.LogError(e, $"Error processing domain event {domainEvent.GetType()}");
                }
            }
