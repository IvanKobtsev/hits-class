...
namespace Team13.WebApi.Swagger;
...
public class ValidationProblemDetailsSchemaProcessor : ISchemaProcessor
{
    public void Process(SchemaProcessorContext context)
    {
        ...
        var errorsProperty = context
            .Schema.ActualProperties.Select(x => x.Value)
            .FirstOrDefault(x =>
                x.Name.ToLower() == nameof(ValidationProblemDetails.Errors).ToLower()
            );
