...
public class FileService
{
    ...
    public FileService(HitsClassDbContext dbContext, IConfiguration configuration)
    {
    ...
    public async Task<PagedResult<FileInfoDto>> Get(SearchFileDto searchDto)
    {
        ...
        if (!string.IsNullOrEmpty(searchDto.FileName))
            dbFiles = dbFiles.Where(x =>
                x.FileName.ToLower().Contains(searchDto.FileName.ToLower())
            );
