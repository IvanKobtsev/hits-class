...
public partial class SetupAudit
{
    private static partial HitsClassDbContext CreateAuditDbContext(HitsClassDbContext dbContext)
    {
        ...
        return new HitsClassDbContext(
            new DbContextOptionsBuilder<HitsClassDbContext>().UseNpgsql(conn).Options,
    ...
    private static void SetupSavingToSerilog()
    {
        Configuration
            .Setup()
            .UseSerilog(config =>
                config.LogLevel(Audit.Serilog.LogLevel.Debug).Message(CreateAuditMessageForSerilog)
            );
    }
}
