...
public static partial class SetupDatabase
{
    ...
    {
        await using var context = serviceProvider.GetRequiredService<HitsClassDbContext>();
    ...
    public static void AddDatabase(WebApplicationBuilder builder)
    {
            ...
            (provider, opt) =>
            {
                opt.UseNpgsql(
                        ...
                        builder =>
                            HitsClassDbContext
                                .MapEnums(builder)
                                .EnableRetryOnFailureWithAdditionalErrorCodes()
                    )
                    .WithLambdaInjection()
        ...
        services
            .AddScoped<Func<HitsClassDbContext>>(provider => () => CreateDbContext(provider))
            .RegisterRetryHelper();
