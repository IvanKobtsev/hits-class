...
public partial class BasicApiTests : ComponentTestBase
{
    ...
    private async Task GenerateCSharpHttpClient(string inputPath, string outputDirPath)
    {
        await RunScriptFromFrontendFolder(
            $"yarn nswag openapi2csclient /input:{inputPath} /output:{outputDirPath}/GeneratedClient.cs /templateDirectory:../webapi/src/Team13.HitsClass.Http/template /namespace:Team13.HitsClass.Http.Generated /generateClientInterfaces:true /clientBaseClass:BaseClient /exposeJsonSerializerSettings:true /generateUpdateJsonSerializerSettingsMethod:false /clientBaseInterface:IBaseClient /jsonLibrary:SystemTextJson"
        );
        await RunScriptFromFrontendFolder(
